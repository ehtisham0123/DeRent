...
function HiredBorrowers({ match, location }) {
  ...
  useEffect(() => {
    ...
    let getBorrowersData = async () => {
      await axios
        .get(`${process.env.React_App_Url}/vendor/products/hired-products/${id}`, {
          headers: {
            token: token,
          }, 
        })
  ...
  const paginate = (pageNumber) => setCurrentPage(pageNumber);
   ...
   const deleteEnrollment = async (borrower_id) => {
    await axios
      .delete(`${process.env.React_App_Url}/vendor/products/enrollment/product/${id}/borrower/${borrower_id}`,
      {
        headers: {
          token: token,
        },
        ...
        >
          <thead>
            <tr role="row">
              ...
              <th style={{ minWidth: "100px" }}>Borrower Name</th>
